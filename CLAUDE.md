...
- 项目迭代记录见 [CHANGE.md](./CHANGE.md)（按日期追加需求与整改条目）（当前迭代：Live2D character 模型 v2 重制——真网格切割 + 等比组装修复拉伸/错位/直线裁切，承接 Live2D 多模型按角色配置；`npm run desktop` 桌面模式 / `npm start` 网页模式；详见 CHANGE.md 2026-09-11 条目）。
