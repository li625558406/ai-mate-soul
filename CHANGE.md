## 2026-09-12 — 根治：视频首帧复刻参考图静态照（构图继承利用）

### 改动主题
用户反馈改用 `role: reference_image` 后视频首帧仍是参考图（白底半身照）。经 8+ 实验定位根因为官方机制而非请求缺陷。

### 根因
官方文档明确：多模态参考继承"角色形象、视觉风格、**画面构图**"，且无参数可关闭构图继承。参考图是白底半身照时，模型大概率复刻该构图开场。附带发现：相同 payload 在不同 HTTP 客户端下结果对立（python urllib 场景开场 vs Node fetch 照片开场，含 UA 伪装仍失败），疑似服务端按客户端分流管线，措辞对抗不可靠。

### 解法（不对抗，利用构图继承）
- 用 Seedream 图生图把头像预生成"生活化实拍自拍构图"参考图（同一人物 + 举手机自拍动作 + 书房场景），存 `data/characters/<id>/video_ref.jpg`——即使构图被复刻开场，画面也是自然场景照
- `src/core/CharacterManager.js`：新增 `getVideoRefPath()`（video_ref.* 匹配）
- `src/services/VideoService.js`：参考图优先级改为 video_ref > avatar > reference；移除临时请求体 dump 调试代码

### 遗留
- video_ref.jpg 为离线预生成，新角色如需同效果须补该文件（无则自动回退头像）
- HTTP 客户端差异导致生成行为不同 Ark 侧未证实原因，仅记录现象
